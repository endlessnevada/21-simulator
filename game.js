// ========================
// GIOCO BLACKJACK + PROBABILITÀ LIVE
// ========================
...
function renderPlayerHand(index) {
  ...
  // Disabilita pulsanti se il giocatore ha fatto stand o sballato
  if (!player.isCPU) {
    const controls = div.querySelector('.controls');
    controls.querySelectorAll('button').forEach(btn => btn.disabled = player.stand);
    // Aggiorna probabilità live
    calculateLiveProbabilities();
  }
}
...
function renderDealer(showAll=false) {
  ...
  dealer.hand.forEach((card,index)=>{
    const img = document.createElement('img');
    img.src = (index===1 && !showAll) ? 'cards/back.png' : `cards/${getCardFilename(card)}`;
    img.className='card';
    handDiv.appendChild(img);
  });
...
function showResults(){
  ...
  players.forEach(player=>{
    ...
    if(playerScore>21) statusDiv.textContent = player.isCPU ? 'CPU ha perso!' : 'Hai perso!';
    else if(dealerScore>21 || playerScore>dealerScore) statusDiv.textContent = player.isCPU ? 'CPU ha vinto!' : 'Hai vinto!';
    else if(playerScore===dealerScore) statusDiv.textContent = 'Pareggio!';
    else statusDiv.textContent = player.isCPU ? 'CPU ha perso!' : 'Hai perso!';
  });
}

// ========================
// INTEGRAZIONE PROBABILITÀ LIVE
// ========================
async function calculateLiveProbabilities(showDetails=false){
  if(players[0].hand.length<2 || dealer.hand.length<1) return;
  const numDecks=parseInt(document.getElementById('numDecks').value)||1;
  const playerHand = players[0].hand.map(cardValueOnly);
  const dealerCard = cardValueOnly(dealer.hand[0]);

  // Scrive valori temporanei nei campi della funzione esistente
  document.getElementById('c1').value = playerHand[0];
  document.getElementById('c2').value = playerHand[1];
  document.getElementById('dealer').value = dealerCard;
  document.getElementById('showDetails').checked = showDetails;

  await calculate(); // richiama il motore di probabilità esistente
}
...
document.getElementById('resetBtn').addEventListener('click',()=>{
  ...
  // Pulisce output probabilità
  ['output','tableStay','tableHitAgain','decisionSummary'].forEach(id=>document.getElementById(id).innerHTML='');
